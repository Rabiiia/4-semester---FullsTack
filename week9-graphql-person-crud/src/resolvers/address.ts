import { Person, Category, Rating, Context } from '../types';
export default {
  ...
  // The Category resolver will be called when we query for a category, and the Category.books field will be resolved.
    people: (parent:Person, _args:never, context:Context) => context.people.filter((person) => person.addressId === parent.id),
}